...
import com.nicolas.shebangscashier.common.ReceiveTransferDataAdapter;
...
public class ReceiveTransferActivity extends BaseActivity implements View.OnClickListener {
    ...
    private ReceiveTransferViewModel viewModel;
    private ReceiveTransferDataAdapter adapter;
    private CheckBox checkAll;
    private boolean isManualCheckAll = false;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_receive_tranfer);
        this.viewModel = new ViewModelProvider(this).get(ReceiveTransferViewModel.class);

        this.drawerLayout = findViewById(R.id.drawer_layout);
        this.user = findClickView(R.id.user);
        this.updateUser();
        this.numCount = findViewById(R.id.numCount);
        this.checkAll = findViewById(R.id.checkBox);
        this.checkAll.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isManualCheckAll) {
                    isManualCheckAll = false;
                    return;
                }
                viewModel.setItemAllChoice(isChecked);
        ...
        this.adapter = new ReceiveTransferDataAdapter(this, viewModel.getDataList(), new ReceiveTransferDataAdapter.OnOperateListener() {
            @Override
            ...
            @Override
            public void onOperate3(int position) {      //某个商品选中状态改变
                transferItemStatusChange();
            }
        });
        ...
        viewModel.getQueryResult().observe(this, new Observer<OperateResult>() {
            ...
            public void onChanged(OperateResult operateResult) {
                ...
                if (operateResult.getError() != null) {
                    Toast.makeText(ReceiveTransferActivity.this, operateResult.getError().getErrorMsg(), Toast.LENGTH_SHORT).show();
                }
        ...
        viewModel.getQueryGoodsByGoodsIdResult().observe(this, new Observer<OperateResult>() {
            ...
            public void onChanged(OperateResult operateResult) {
                ...
                if (operateResult.getError() != null) {
                    BruceDialog.showPromptDialog(ReceiveTransferActivity.this,
        ...
        viewModel.getReplenishmentSubmitResult().observe(this, new Observer<OperateResult>() {
            ...
            public void onChanged(OperateResult operateResult) {
                if (operateResult.getSuccess() != null) {
                    Toast.makeText(ReceiveTransferActivity.this, "补货成功", Toast.LENGTH_SHORT).show();
                }
                if (operateResult.getError() != null) {
                    BruceDialog.showPromptDialog(ReceiveTransferActivity.this,
                            operateResult.getError().getErrorMsg());
        ...
        //监听接收调货提交
        viewModel.getSubmitReceiveTransferResult().observe(this, new Observer<OperateResult>() {
            @Override
            public void onChanged(OperateResult operateResult) {
                if (operateResult.getSuccess() != null) {
                    adapter.notifyDataSetChanged();
                    Toast.makeText(ReceiveTransferActivity.this, "提交成功", Toast.LENGTH_SHORT).show();
                }
                if (operateResult.getError() != null) {
                    BruceDialog.showPromptDialog(ReceiveTransferActivity.this,
                            operateResult.getError().getErrorMsg());
     ...
     * 某个商品选中状态改变
     */
    private void transferItemStatusChange() {
        adapter.notifyDataSetChanged();
        isManualCheckAll = true;
        checkAll.setChecked(viewModel.isAllItemSelect());
    }
    ...
    private void showSalespersonDialog() {
        BruceDialog.showSingleChoiceDialog(R.string.employees, ReceiveTransferActivity.this, MyKeeper.getInstance().getEmployeesName(),
    ...
    private void showAcceptanceShop() {
        BruceDialog.showEditInputDialog(getString(R.string.input_acceptance), getString(R.string.shopID), InputType.TYPE_CLASS_TEXT,
                ReceiveTransferActivity.this,
    ...
    private void transferTimeChoice(String itemValue) {
        ...
        DateTimePickerDialog.showDateSlotPickerDialog(ReceiveTransferActivity.this, start, end, new DateTimePickerDialog.OnDateTimeSlotPickListener() {
            @Override
