...
import com.nicolas.shebangscashier.common.ReceiveTransferData;
...
public class ReceiveTransferViewModel extends ViewModel {
    ...
    private ReceiveTransferCondition condition;
    ...
    private MutableLiveData<OperateResult> submitReceiveTransferResult;
    private List<ReceiveTransferData> dataList;

    private ReceiveTransferData currentData;
    private ReplenishmentInformation information;

    private int deletePosBak;

    public ReceiveTransferViewModel() {
        ...
        this.replenishmentSubmitResult = new MutableLiveData<>();
        this.submitReceiveTransferResult = new MutableLiveData<>();
        this.condition = new ReceiveTransferCondition();
    }

    public ReceiveTransferCondition getCondition() {
        return condition;
    }
    ...
    public LiveData<OperateResult> getSubmitReceiveTransferResult() {
        return submitReceiveTransferResult;
    }
    ...
    public List<ReceiveTransferData> getDataList() {
        return dataList;
    }

    public ReceiveTransferData getCurrentData() {
        return currentData;
    }
    ...
    private void queryTransfer() {
        ...
        vo.url = ManageInterface.TransferGoodsSearchTo;
        ...
        if (!TextUtils.isEmpty(condition.branchTo)) {             //接收分店
            parameters.put("branchFrom", condition.branchTo);
        }
     ...
     * 确认接收调货
     */
    public void submitReceiveTransfer() {
        CommandVo vo = new CommandVo();
        vo.typeEnum = CommandTypeEnum.COMMAND_MANAGE;
        vo.url = ManageInterface.TransferGoodsListOK;
        vo.contentType = HttpHandler.ContentType_APP;
        vo.requestMode = HttpHandler.RequestMode_GET;
        Map<String, String> parameters = new HashMap<>();
        StringBuilder builder = new StringBuilder();
        for (ReceiveTransferData data : dataList) {
            if (data.isSelect) {
                builder.append(data.id).append(",");
            }
        }
    ...
    private Invoker.OnExecResultCallback callback = new Invoker.OnExecResultCallback() {
        ...
        public void execResult(CommandResponse result) {
            switch (result.url) {
                case ManageInterface.TransferGoodsSearchTo:
                    if (result.success) {
                        try {
                            ...
                            if (array.length() == 0) {
                                ...
                                for (int i = 0; i < array.length(); i++) {
                                    ReceiveTransferData item = new ReceiveTransferData(array.getString(i));
                                    dataList.add(item);
                                }
                ...
                case ManageInterface.OrderByBranch:
                    ...
                    break;
                case ManageInterface.TransferGoodsListOK:
                    if (result.success) {
                        submitReceiveTransferResult.setValue(new OperateResult(new OperateInUserView(null)));
                    } else {
                        submitReceiveTransferResult.setValue(new OperateResult(new OperateError(result.code, result.msg, null)));
                    }
                    break;
                default:
                    break;
     ...
     */
    public void setItemAllChoice(boolean isChecked) {
        for (ReceiveTransferData data : dataList) {
    ...
    public class ReceiveTransferCondition {
