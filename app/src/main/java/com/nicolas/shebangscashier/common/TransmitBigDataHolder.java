 ...
 */
public class TransmitBigDataHolder {

    private Map<String, WeakReference> dataList = new HashMap<>();

    private static volatile TransmitBigDataHolder holder;

    public static TransmitBigDataHolder getInstance(){
        if (holder==null){
            synchronized (TransmitBigDataHolder.class){
                if (holder==null){
                    holder = new TransmitBigDataHolder();
                }
            }
        }
        return holder;
